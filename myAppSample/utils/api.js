...
export const getCurrentTemp = async () => {
  ...
  const response = await fetch(
    `https://morning-garden-87436.herokuapp.com/`
  );
  let { CurrentHumid, CurrentTemp } = await response.json();
...
export const getNextTemp = async () => {
  const response = await fetch(
    `https://morning-garden-87436.herokuapp.com/iot`
  );
  let { NextTemp } = await response.json();
  return {
   nextTemp: NextTemp
  };
};

export const getTemp = async (temp, humid) => {
  const response = await fetch(
    `https://morning-garden-87436.herokuapp.com/${temp}/${humid}`
  );
  let { NextTemp } = await response.json();
  return {
   nextTemp: NextTemp
